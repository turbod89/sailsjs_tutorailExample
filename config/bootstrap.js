...
module.exports.bootstrap = async function(done) {
  ...
  const users = await User.createEach([
    ...
    { emailAddress: 'ghouse@example.com', fullName: 'Gregory House', isSuperAdmin: false, password: await sails.helpers.passwords.hashPassword('abc123') },
    { emailAddress: 'jwilson@example.com', fullName: 'James Wilson', isSuperAdmin: false, password: await sails.helpers.passwords.hashPassword('abc123') },
  ]).fetch();
  ...
  const things = await Thing.createEach([
    ...
    {owner: users[Math.floor(users.length * Math.random())].id, label: '3-years-old Catan game'},
    {owner: users[Math.floor(users.length * Math.random())].id, label: 'Deck of cards'},
    {owner: users[Math.floor(users.length * Math.random())].id, label: 'Coffe machine'},
    {owner: users[Math.floor(users.length * Math.random())].id, label: 'Two garden chairs'},
    {owner: users[Math.floor(users.length * Math.random())].id, label: 'Tucan\'s jail'},
    {owner: users[Math.floor(users.length * Math.random())].id, label: 'Paddel'},
